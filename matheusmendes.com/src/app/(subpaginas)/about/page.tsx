         ...
         {dataTimeLine.map(({smallText,text,title},index)=>{
          ...
          return(
            ...
            <div id='container' className={`py-2 px-2 relative sm:w-1/2 w-full ${index % 2 === 0?"sm:left-0":"sm:left-1/2"}`}>
             <div className={`hidden sm:block w-14 h-14  absolute rounded-full bg-white ${index % 2 === 0? '-right-7':'-left-7'} top-8   z-10`}></div>
           ...
           {index <= lengthDataTime && (
              <div key={`bar-${index}`} className={`hidden sm:flex w-1 h-full bg-white mx-auto absolute top-0  ${index % 2 === 0? 'left-1/2' : 'right-1/2'}`}     />
           )}
           </>
          )
         })}
